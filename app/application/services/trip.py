...
class TripService:
    ...
    async def create(self, trip_data: TripCreate, user_id: int) -> Trip:
    ...
    async def list_for_user(
        ...
        stmt = (
            select(Trip)
            .where(Trip.user_id == user_id, Trip.deleted_at.is_(None))  # type: ignore
            .offset(skip)
            .limit(limit)
        )
    ...
    async def get(self, trip_id: int, user_id: int) -> Trip:
        return await self._get_own_trip(trip_id, user_id)

    async def patch(self, trip_id: int, user_id: int, trip_data: TripUpdate) -> Trip:
